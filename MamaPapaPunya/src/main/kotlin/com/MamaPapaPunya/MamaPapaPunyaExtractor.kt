...
object MamaPapaPunyaExtractor : MamaPapaPunya() {

    // ================== MAMAPAPA MOVIEBOX SOURCE ==================
    suspend fun invokeMamaPapaMoviebox(
        title: String,
        year: Int?,
        season: Int?,
        episode: Int?,
        ...
        val items = tryParseJson<MamaPapaMovieboxSearch>(searchRes)?.data?.items ?: return
        ...
        val streams = tryParseJson<MamaPapaMovieboxStreams>(playRes)?.data?.streams ?: return
        ...
        if (id != null) {
            ...
            app.get(subUrl, referer = validReferer).parsedSafe<MamaPapaMovieboxCaptions>()?.data?.captions?.forEach { sub ->
    ...
    // ================== MAMAPAPA DEWASA (DRAMAFULL) ==================
    @Suppress("UNCHECKED_CAST") 
    suspend fun invokeMamaPapaDewasa(
        title: String,
        year: Int?,
        season: Int?,
        episode: Int?,
        ...
        // 1. PEMBERSIHAN JUDUL
        val cleanQuery = MamaPapaHelper.normalizeQuery(title)
        ...
        try {
            ...
            val searchRes = app.get(searchUrl, headers = MamaPapaHelper.headers).parsedSafe<MamaPapaDewasaSearchResponse>()
            
            // 2. PENCOCOKAN JUDUL
            val matchedItem = searchRes?.data?.find { item ->
                val itemTitle = item.title ?: item.name ?: ""
                MamaPapaHelper.isFuzzyMatch(title, itemTitle)
            ...
            // 3. LOAD HALAMAN FILM
            val doc = app.get(targetUrl, headers = MamaPapaHelper.headers).document
            ...
            val docPage = app.get(targetUrl, headers = MamaPapaHelper.headers).document
            ...
            val jsonResponseText = app.get(signedUrl, referer = targetUrl, headers = MamaPapaHelper.headers).text
