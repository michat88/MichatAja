...
import com.AdiDewasa.AdiDewasaExtractor.invokeAdiDewasa
import com.AdiDewasa.AdiDewasaExtractor.invokeAdimoviebox
...
import com.AdiDewasa.AdiDewasaExtractor.invokeWatchsomuch
...
open class AdiDewasa : TmdbProvider() {
    ...
    // Main Page menggunakan TMDB agar compatible dengan semua extractor
    override val mainPage = mainPageOf(
        "$tmdbAPI/trending/all/day?api_key=$apiKey" to "Trending Now",
        "$tmdbAPI/discover/movie?api_key=$apiKey&sort_by=popularity.desc" to "Popular Movies",
        "$tmdbAPI/discover/tv?api_key=$apiKey&sort_by=popularity.desc" to "Popular TV Shows",
        "$tmdbAPI/discover/movie?api_key=$apiKey&with_original_language=ko&sort_by=popularity.desc" to "Popular K-Movies",
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        return if (type == TvType.TvSeries) {
            ...
            val episodes = res.seasons?.mapNotNull { season ->
                app.get("$tmdbAPI/${data.type}/${data.id}/season/${season.seasonNumber}?api_key=$apiKey")
                    .parsedSafe<MediaDetailEpisodes>()?.episodes?.map { eps ->
                        newEpisode(
                            data = LinkData(
                                data.id, res.external_ids?.imdb_id, res.external_ids?.tvdb_id, data.type, eps.seasonNumber, eps.episodeNumber,
                                title = title, year = season.airDate?.split("-")?.first()?.toIntOrNull(), orgTitle = orgTitle, airedYear = year,
                                lastSeason = lastSeason, epsTitle = eps.name, airedDate = res.releaseDate ?: res.firstAirDate
                            ).toJson()
                        ) {
            ...
            newTvSeriesLoadResponse(title, url, TvType.TvSeries, episodes) {
                this.posterUrl = poster; this.backgroundPosterUrl = bgPoster; this.year = year; this.plot = res.overview; this.tags = keywords.takeIf { !it.isNullOrEmpty() } ?: genres
                this.score = Score.from10(res.vote_average?.toString()); this.showStatus = getStatus(res.status); this.recommendations = recommendations
                addTrailer(trailer); addTMDbId(data.id.toString()); addImdbId(res.external_ids?.imdb_id)
            }
        } else {
            newMovieLoadResponse(title, url, TvType.Movie, LinkData(data.id, res.external_ids?.imdb_id, res.external_ids?.tvdb_id, data.type, title = title, year = year, orgTitle = orgTitle, airedDate = res.releaseDate ?: res.firstAirDate).toJson()) {
                this.posterUrl = poster; this.backgroundPosterUrl = bgPoster; this.comingSoon = isUpcoming(releaseDate); this.year = year; this.plot = res.overview; this.duration = res.runtime
                this.tags = keywords.takeIf { !it.isNullOrEmpty() } ?: genres; this.score = Score.from10(res.vote_average?.toString()); this.recommendations = recommendations
                addTrailer(trailer); addTMDbId(data.id.toString()); addImdbId(res.external_ids?.imdb_id)
            }
        }
    }
    ...
    // --- DATA CLASS FOR TMDB API ---
    data class LinkData(val id: Int? = null, val imdbId: String? = null, val tvdbId: Int? = null, val type: String? = null, val season: Int? = null, val episode: Int? = null, val aniId: String? = null, val animeId: String? = null, val title: String? = null, val year: Int? = null, val orgTitle: String? = null, val isAnime: Boolean = false, val airedYear: Int? = null, val lastSeason: Int? = null, val epsTitle: String? = null, val jpTitle: String? = null, val date: String? = null, val airedDate: String? = null, val isAsian: Boolean = false, val isBollywood: Boolean = false, val isCartoon: Boolean = false)
    data class Data(val id: Int? = null, val type: String? = null, val aniId: String? = null, val malId: Int? = null)
